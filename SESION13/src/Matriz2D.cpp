...
Matriz2D::Matriz2D(const char *nombre)
{
	...
	string cad_inicial;
	f >> cad_inicial;
	cout<<"cad_inicial: "<<cad_inicial<<endl;

	if(cad_inicial!="MATRIZ2D")
	{
		cerr << "Error: El fichero " << nombre << " no es un fichero de matriz" << endl;
		exit(1);
	}
	...
	cout<<"Reservada memoria con "<<fils<<" filas y "<<cols<<" columnas"<<endl;
...
/*************************************************************************/
// Operador de asignación que ecualiza toda la matriz a un valor dado
// Parámetros: valor (TipoBaseMatriz)
...
/*************************************************************************/
// Operador unario +=
// Recibe un valor de TipoBaseMatriz y suma el valor a todas las casillas.
...
/*************************************************************************/
// Operador unario -=
// Recibe un valor de TipoBaseMatriz y resta el valor a todas las casillas.
...
{
	Matriz2D aux(m);
	os << setw(6) << aux.NumFilas() << setw(6) << aux.NumColumnas() << endl;

	for (int i = 0; i < aux.NumFilas(); i++)
	{
		for (int j = 0; j < aux.NumColumnas(); j++)
			os << setw(6) << aux(i, j);
		os << endl;
	}
	return os;
...
istream &operator>>(istream &is, Matriz2D &m)
{
	int filas, columnas;
	is >> filas >> columnas;
